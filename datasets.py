...
def make_rt_gender(batch_size, device=-1, vectors=None, base_path="", suffix="",extrasuffix="",domain="", oodname="", topics=False):
  TEXT = data.Field(include_lengths=True, lower=True)
  LABEL = data.LabelField()
  TOPICS = data.Field(sequential=True, use_vocab=False, preprocessing=data.Pipeline(lambda x:float(x)), tensor_type=torch.cuda.FloatTensor, batch_first=True)
  train = data.TabularDataset(path=base_path+"/train"+suffix+extrasuffix+".txt", format="tsv", fields=[('id', None), ('text',TEXT), ('label', LABEL), ('topics', TOPICS)])
  val = data.TabularDataset(path=base_path+"/valid"+suffix+".txt", format="tsv", fields=[('id', None), ('text',TEXT), ('label', LABEL)])
  test = data.TabularDataset(path=base_path+"/test"+suffix+".txt", format="tsv", fields=[('id', None), ('text',TEXT), ('label', LABEL)])
  TEXT.build_vocab(train, vectors=vectors, max_size=30000)
  LABEL.build_vocab(train)
  print (LABEL.vocab.stoi)
  train_iter, val_iter, test_iter = data.BucketIterator.splits((train, val, test), batch_sizes=(batch_size, 256, 256), device=device, repeat=False, sort_key=lambda x: len(x.text))
